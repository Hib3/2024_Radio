...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 疎通確認ボタンのクリックイベントを追加
  document.getElementById("check-connectivity").addEventListener("click", function () {
      checkConnectivity();
  });
});
...
function checkConnectivity() {
  // 各ラジオボタンから疎通失敗時のクラスを削除
  const radioButtons = document.querySelectorAll(".radiobox-tremolo");
  radioButtons.forEach(button => {
      button.classList.remove("failed-channel");
  });

  channels.forEach((channel, index) => {
      // 正規表現でURLを抽出
      const urlMatch = channel.match(/src='(.*?)'/);
...
// 疎通失敗時にラジオボタンを赤色に変更する関数
function markAsFailed(index) {
  const radioButton = document.getElementById(`radio-${index + 1}`);
  if (radioButton) {
      radioButton.classList.add("failed-channel");
  }
}
